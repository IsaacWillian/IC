...
threshold_params = range(100, 250,30)
elem_params = [10, 12, 14, 16]
...
for filename_original,filename_gold in zip(dir_original,dir_gold):
    ...
    for dilation in (dilation_params):
        for threshold in (threshold_params):
            for elem in (elem_params):        
                    ...
                    results_dict['results'][(dilation, threshold, elem)] = {
                    'tp':tp,
                    'tn':tn,
                    'fp':fp,
                    'fn':fn,
